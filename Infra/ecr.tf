...
locals {
  docker_login_command              = "aws ecr get-login-password --region ${var.region} --profile personal| docker login --username AWS --password-stdin 918822628996.dkr.ecr.${var.region}.amazonaws.com"
  docker_build_command              = "docker build -t ${aws_ecr_repository.ecr.name} ../"
  docker_tag_command                = "docker tag ${aws_ecr_repository.ecr.name}:latest 918822628996.dkr.ecr.${var.region}.amazonaws.com/${aws_ecr_repository.ecr.name}:latest"
  docker_push_command               = "docker push 918822628996.dkr.ecr.${var.region}.amazonaws.com/${aws_ecr_repository.ecr.name}:latest"
}
